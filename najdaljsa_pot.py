...
def simulacija_med_dvema_vozliscema(st_vozlisc, min_utez, max_utez, st_grafov = 15):
    ...
    with open(datoteka, "w") as f:
        writer = csv.writer(f)
        writer.writerow(["Število povezav", "najvecja dovoljena utez", "najmanjsa dovoljena utez", "Graf", "Dolžina najdaljše poti med prvim in zadnjim vozliscem", "Čas"])
        for i in range(1, st_grafov+1):
...
def simulacija_st_ponovitev(st_ponovitev, preskok = 5, st_grafov = 1, st_vozlisc = 8, st_barv = 11):
    ...
    with open(datoteka, "w") as f:
        writer = csv.writer(f)
        writer.writerow(["Število povezav", "Graf", "Število ponovitev","Dolžina najdaljše poti - Color coding", "Čas", "Dolžina najdaljše poti - DFS", "Čas"])
        for i in range(1, st_grafov+1):
            G = nakljucni_graf_neutezen(st_vozlisc)
            st_pov = stevilo_povezav(G)
            start2 = time.time()
